...
from fast_knn_nmt.custom_fairseq.train.utils import move_to_cuda
...
def _main(args, output_file):
    ...
    task.load_dataset(args.gen_subset, incremental=False if args.score_reference else True)

    if args.score_reference:
        assert args.batch_size == 1, "only support batch-size==1 iff score_reference"
        ...
        for i, sample_id in enumerate(sample["id"].tolist()):
            ...
            for j, hypo in enumerate(hypos[i][: args.nbest]):
                ...
                if not args.quiet:
                    ...
                    if args.score_reference:
                        position_score = hypo["positional_scores"].div(math.log(2)).tolist()
                        if args.ignore_eos:
                            position_score = position_score[: -1]
                        position_scores.extend(position_score)
    ...
    if has_target:
        ...
        if args.score_reference:
            ...
            ppl = 2**(-avg_score)
            print(
                f"Average score/ppl over {len(position_scores)} tokens:{avg_score:.4f}/{ppl:.4f}"
            )
