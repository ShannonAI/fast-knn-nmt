...
from fast_knn_nmt.custom_fairseq.models.graph_transformer import EncoderOut
from fast_knn_nmt.custom_fairseq.train.trainer import move_to_cuda
...
class SequenceGenerator(nn.Module):
    ...
    def generate_batched_itr(self, data_itr, beam_size=None, cuda=False, timer=None):
        ...
        for sample in data_itr:
            # we change this line to move dgl.graph to cuda
            # s = utils.move_to_cuda(sample) if cuda else sample
            s = move_to_cuda(sample) if cuda else sample
        ...
        for step in range(max_len + 1):  # one extra step for EOS marker
            ...
            if reorder_state is not None:
                ...
                encoder_outs = self.model.reorder_encoder_out(
                    encoder_outs, reorder_state, prev_token=tokens[:, step], cur_step=step
                )

            # predict tgt nodes that step_idx==step
            for encoder_out in encoder_outs:
                step_mask = (encoder_out.graph.nodes["tgt"].data["step_idx"] == step)
                encoder_out.graph.nodes["tgt"].data["labels_mask"] = step_mask
...
class EnsembleModel(nn.Module):
    ...
    @torch.jit.export
    def reorder_encoder_out(self, encoder_outs: Optional[List[EncoderOut]], new_order, prev_token=None, cur_step=None):
        """
        ...
        for i, model in enumerate(self.models):
            assert encoder_outs is not None
            new_outs.append(
                model.encoder.reorder_encoder_out(encoder_outs[i], new_order, prev_token, cur_step)
            )
        return new_outs
